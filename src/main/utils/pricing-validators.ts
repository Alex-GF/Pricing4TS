...
export function validateExpression(
  ...
  if (typeof expression === 'string') {
    ...
    validateExpressionSyntax(expression, item);
      ...
      case 'TEXT':
        if (elem.type === 'PAYMENT' && elem.value !== undefined) {
          ...
          for (const paymentMethod of elem.value) {
            if (
              !['CARD', 'GATEWAY', 'INVOICE', 'ACH', 'WIRE_TRANSFER', 'OTHER'].includes(
                paymentMethod
              )
            ) {
              throw new Error(
  ...
  export function validateFeatureAutomationType(
    ...
    if (automationType && typeof automationType !== 'string') {
      ...
      automationType &&
      typeof automationType === 'string' &&
      !isAutomationType(automationType)
    ) {
      throw new Error(
  ...
  export function validatePlanUsageLimits(
    ...
    for (const planUsageLimit of Object.values(usageLimitsModifiedByPlan)) {
      try {
        ...
        globalUsageLimit.value = validateValue(globalUsageLimit, 'usage limit') as
          | string
          | number
          | boolean
          | undefined;
  ...
  export function validatePricingUrls(
    ...
    if (pricingUrls === undefined || pricingUrls === null) {
      ...
      for (const url of pricingUrls) {
        ...
        if (!urlPattern.test(url)) {
          throw new Error(
            `The pricingUrls field must contain only valid URLs. Invalid URL: ${url}`
          );
        }
      }
  ...
  export function validateDocUrl(featureType: FeatureType, docUrl: string | undefined) {
    if (docUrl === undefined || docUrl === null) {
      ...
      if (!urlPattern.test(docUrl)) {
        throw new Error(
          `The docUrl field must be a valid URL with the http or https protocol. Received: ${docUrl}`
        );
      }
  ...
  export function validatePeriodUnit(
    ...
    if (typeof periodUnit !== 'string') {
      throw new Error(
        `The periodUnit field must be one of the following string values: "SEC" | "MIN" | "HOUR" | "DAY" | "MONTH" | "YEAR". Received: ${periodUnit}`
      );
    }
  ...
  export function validateSubscriptionConstraintMinQuantity(
    ...
    if (
      subscriptionConstraintMinQuantity === undefined ||
      subscriptionConstraintMinQuantity === null
    ) {
    ...
    if (typeof subscriptionConstraintMinQuantity !== 'number') {
      throw new Error(
        `The subscriptionConstraintMinQuantity field must be a number. Received: ${subscriptionConstraintMinQuantity}`
      );
    }

    if (subscriptionConstraintMinQuantity < 0) {
      throw new Error(
        `The subscriptionConstraintMinQuantity field must be a positive number. Received: ${subscriptionConstraintMinQuantity}`
      );
    }
  ...
  export function validateSubscriptionConstraintMaxQuantity(
    subscriptionConstraintMaxQuantity: number | undefined,
    minQuantity: number
  ): number {
    if (
      subscriptionConstraintMaxQuantity === undefined ||
      subscriptionConstraintMaxQuantity === null
    ) {
    ...
    if (typeof subscriptionConstraintMaxQuantity !== 'number') {
      throw new Error(
        `The subscriptionConstraintMaxQuantity field must be a number. Received: ${subscriptionConstraintMaxQuantity}`
      );
    }

    if (subscriptionConstraintMaxQuantity < minQuantity) {
      throw new Error(
        `The subscriptionConstraintMaxQuantity field must be greater than or equal to the min quantity. Received: ${subscriptionConstraintMaxQuantity}`
      );
    }
  ...
  export function validateSubscriptionConstraintQuantityStep(
    subscriptionConstraintQuantityStep: number | undefined,
    minQuantity: number
  ): number {
    if (
      subscriptionConstraintQuantityStep === undefined ||
      subscriptionConstraintQuantityStep === null
    ) {
    ...
    if (typeof subscriptionConstraintQuantityStep !== 'number') {
      throw new Error(
        `The subscriptionConstraintQuantityStep field must be a number. Received: ${subscriptionConstraintQuantityStep}`
      );
    }

    if (!(minQuantity % subscriptionConstraintQuantityStep === 0)) {
      throw new Error(
        `The subscriptionConstraintQuantityStep field must be a divisor of, at least, the min quantity. Received: ${subscriptionConstraintQuantityStep}`
      );
    }
...
function validateExpressionSyntax(expression: string, item: string): void {
  // Validate only usage of pricingContext and subscriptionContext with the correct format
  // 1. Detect all used identifiers
  // 2. Only allow pricingContext["features"]["..."], pricingContext["usageLimits"]["..."], subscriptionContext["..."]
  // 3. Only allow logical/comparison operators and parentheses outside of that

  // Extract all word-type identifiers
  ...
  // Regex to find words that are not pricingContext or subscriptionContext
  const forbiddenWordRegex = /\b([a-zA-Z_][a-zA-Z0-9_]*)\b/g;
  // Regex to find all pricingContext and subscriptionContext
  const pricingContextRegex =
    /pricingContext\s*\[\s*["'](features|usageLimits)["']\s*\]\s*\[\s*["'][^"']+["']\s*\]/g;
  const subscriptionContextRegex = /subscriptionContext\s*\[\s*["'][^"']+["']\s*\]/g;

  // 1. Find all valid matches
  const validPricingContextMatches = expression.match(pricingContextRegex) || [];
  const validSubscriptionContextMatches = expression.match(subscriptionContextRegex) || [];

  // 2. Remove valid matches from the expression to analyze the rest
  let exprWithoutContexts = expression;
  for (const m of validPricingContextMatches) {
    exprWithoutContexts = exprWithoutContexts.replace(m, '');
  }
  for (const m of validSubscriptionContextMatches) {
    exprWithoutContexts = exprWithoutContexts.replace(m, '');
  }
  ...
  let match;
  while ((match = forbiddenWordRegex.exec(exprWithoutContexts)) !== null) {
    // Allow boolean and null literals
    if (!['true', 'false', 'null', 'undefined'].includes(match[1])) {
      forbiddenWords.push(match[1]);
    }
  }
  ...
  // 5. Validate that all pricingContext and subscriptionContext usages are well-formed
  // Find any usage of pricingContext or subscriptionContext that is not in the allowed format
  const pricingContextAnyRegex = /pricingContext\s*(\[[^\]]*\])?/g;
  const subscriptionContextAnyRegex = /subscriptionContext\s*(\[[^\]]*\])?/g;

  let m2;
  while ((m2 = pricingContextAnyRegex.exec(expression)) !== null) {
    const full = m2[0];
    // If not in the valid matches, error
    if (!validPricingContextMatches.some(v => v.includes(full))) {
      throw new Error(
        `Invalid usage of pricingContext in ${item} for feature. Only pricingContext["features"]["..."] or pricingContext["usageLimits"]["..."] are allowed.`
      );
    }
  }
  while ((m2 = subscriptionContextAnyRegex.exec(expression)) !== null) {
    const full = m2[0];
    if (!validSubscriptionContextMatches.some(v => v.includes(full))) {
      throw new Error(
        `Invalid usage of subscriptionContext in ${item} for feature. Only subscriptionContext["..."] is allowed.`
      );
    }
  }
}
