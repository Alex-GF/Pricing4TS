...
const OLD_VERSION_SAAS_CSV_PATH = "tests/pricing-service/data/pricing-service-tests-small.csv";
...
describe('Given pricing should return its analytics', () => {
    ...
    afterAll(() => {
        ...
        fs.writeFileSync(errorCSVPath, errorCSVContent.reduce((acc, row) => acc + row.join(",") + "\n", errorCSVHeader.join(",") + "\n"));
    })
            ...
            for (const {pricingPath, expected} of tests) {
                ...
                it(`Get analytics of ${expected}`, async () => {
                    ...
                    try{
                        ...
                        assert.fail(`Error in ${pricingPath}: ${(e as ErrorMessage).message}`);
                    }
                });
            }}
        );
